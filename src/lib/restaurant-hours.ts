 ...
 */
export function resolveStatus(restaurant: Restaurant, day: number, minutes: number): OpenStatus {
...
export function msUntilNextBoundary(now: Date): number {
  ...
  const next = low < BOUNDARIES.length ? BOUNDARIES[low] : BOUNDARIES[0] + MINUTES_PER_WEEK;
