...
function legacyRender(block: string) {
  ...
  const checked = selectedHostel?.wardens.map((warden) => selectedWardens.includes(warden.name));
...
function indexedRender(block: string) {
  ...
  const checked = selectedHostel?.wardens.map((warden) => selectedWardenSet.has(warden.name));
...
console.log("\nequivalence");
assertEquivalent(
  "every block produces identical derived state",
  BLOCKS,
  legacyRender,
  indexedRender,
);
